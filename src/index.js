    ...
    const checkNode = node => {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (currentElem.nodeType === 1) {
                if (stats.tags[currentElem.tagName]) {
                    stats.tags[currentElem.tagName]++
                } else {
                    stats.tags[currentElem.tagName] = 1
                }

                for (const elem of currentElem.classList) {
                    if (stats.classes[elem]) {
                        stats.classes[elem]++
                    } else {
                        stats.classes[elem] = 1
                    }
                }
                checkNode(currentElem)
            }
